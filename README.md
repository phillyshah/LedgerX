...
Tapping an **already-matched** line item shows what it's matched to — vendor, date, amount, property, and the receipt image if one was attached — with an **Undo** button right there, instead of an empty picker with nothing left to select.

Each statement's card in the list shows a **progress ring** around the card icon with its match percentage at a glance — gray at 0%, amber partway, emerald once every line item is matched — so you can scan for what still needs attention without opening each one.
