...
- **Version bump**: Every merge to main MUST increment the version shown on the login screen (`src/components/AuthForm.tsx` — currently `v3.2`) AND `package.json`. Use semver: patch for fixes (`3.1` → `3.1.1`), minor for features (`3.1` → `3.2`), major for breaking changes.
