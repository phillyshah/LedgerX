...
2. Um administrador envia uma fatura de cartão — como exportação CSV, ou como PDF/foto que o LedgerX lê automaticamente — em **LedgerX Labs → Conciliação de Cartão de Crédito**. Por padrão, uma fatura não pertence a uma única residência, já que um mesmo cartão costuma cobrir mais de um imóvel — mas você pode opcionalmente escolher qual residência (ou residências) uma fatura específica realmente cobre. Isso restringe a associação apenas aos recibos daquela residência (além de qualquer recibo sem categoria em qualquer lugar e do que estiver na caixa de entrada de e-mail), o que torna a associação automática visivelmente mais decisiva, já que ela deixa de comparar com as cobranças de todas as residências participantes ao mesmo tempo. Se nenhuma for selecionada, a fatura se comporta exatamente como antes. Uma fatura restrita dessa forma exibe uma pequena etiqueta "Restrito a: ..." onde quer que apareça.
3. Um administrador (completo ou de residência) abre a fatura e percorre seus lançamentos, associando cada um a um recibo já cadastrado no LedgerX. A correspondência considera **todas as propriedades inscritas no experimento** — para que os recibos de uma propriedade apareçam, um administrador ativa **Labs: Conciliação de Cartão de Crédito** para aquela residência em Gerenciar Residências → Recursos. Uma vez inscrita, qualquer administrador pode associar uma cobrança ao recibo certo, não importa a qual propriedade ele pertença. (Um administrador de residência vê suas próprias submissões e as de usuários comuns e prestadores; recibos enviados por outros administradores ou pelo dono da conta ficam fora da sua lista de candidatos. Um administrador completo vê tudo.) Um recibo é sugerido sempre que o **valor** coincide com a cobrança — a data e o fornecedor afetam apenas a ordenação, então um recibo registrado semanas depois ou com o nome do fornecedor alterado ainda aparece. Um botão permanente de **Associação automática** resolve os casos óbvios com um toque, e qualquer associação pode ser desfeita. Se as sugestões não incluírem o recibo que você procura, use a caixa de busca para **navegar pela lista completa de recibos** e escolher o certo manualmente.
