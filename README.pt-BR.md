...
3. O administrador dessa residência (ou um administrador completo) abre a fatura e percorre seus lançamentos, associando cada um a um recibo já cadastrado no LedgerX. A correspondência considera **todas as propriedades que a fatura pode cobrir**, não apenas uma residência. As sugestões de associação são ordenadas por valor, data e fornecedor — um botão permanente de **Associação automática** resolve os casos óbvios com um toque, e qualquer associação pode ser desfeita. Se as sugestões não incluírem o recibo que você procura, use a caixa de busca para **navegar pela lista completa de recibos** e escolher o certo manualmente.
